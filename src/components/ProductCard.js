...
export const ProductCard =({item, addLot, removeLot, addItem, removeItem})=>{
   ...
   const [unitTypeLot, setUnitTypeLot] = useState(false)
    return <div key={item._id} className="p-4 bg-white m-2 rounded-xl h-50 w-40">
                  ...
                  <div key={item._id+3}>
                    <button onClick={unitTypeLot? removeLot: removeItem}>-</button>
                    {!unitTypeLot?cart[item._id].count:cart[item._id].count/Math.floor(item.min_lot_size)}
                    <button onClick={unitTypeLot? addLot: addItem}>+</button>
                    </div>
                    :<button key={item._id+4} className="bg-secondary text-white" onClick={unitTypeLot?addLot:addItem}>add to cart</button>
                  ):""
